...
public:
    ...
    void on_recv_top(cb<void(int mes, int auxs)> cb) { top_cb_ = std::move(cb); }
    void on_recv_init_done(cb<void(const vec<input_data>&)> cb) { done_cb_ = std::move(cb); }
